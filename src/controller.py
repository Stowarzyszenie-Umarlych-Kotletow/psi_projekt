...
class InvaliFindArgsException(Exception):
...
class Controller:
    def __init__(self):
        ...
        # finding phrase that tells you what is currently being searched for
        # (if not None, then it means we are collecting found/not found responses)
        self._finding = None

        # peers that not yet responded to find
        self._peers_not_responded = set()

        # dict of keys (file_hash, provider_ip) and of values FoundResponse
        self._found_responses = dict()

        # common lock for finding, found_responses and peers_not_responded
    ...
    def get_peer(self, ip):
        return self._known_peers.get(ip)

    def find(self, file_name: str = None, file_hash: str = None):
        if file_name is None and file_hash is None:
            raise InvaliFindArgsException("Filename and filehash cannot be both of type None")
        ...
        if file_name == "" and file_hash == "":
            raise InvaliFindArgsException("Filename and filehash cannot be both empty")

        if file_hash != "" and len(file_hash) != 64:
            raise InvaliFindArgsException("FileHash does not look like sha256sum")

        self._finding_lock.acquire()
        self._finding = (file_name, file_hash)
        self._found_responses = dict()
        self._peers_not_responded = set(self.get_peers().keys())
        self._finding_lock.release()
        ...
        for retry in range(FINDING_RETRIES):
            if len(self._peers_not_responded) != 0:
                self._broadcast_socket.send(find_datagram.to_bytes())
                time.sleep(FINDING_TIME)

        # delete peers that did not respond
        self._known_peers_lock.acquire()
        for peer_ip in self._peers_not_responded:
            self._known_peers.pop(peer_ip)
    ...
    def found_callback(self, datagram_bytes: bytes, address: Tuple[str, int]):
        ...
        self._finding_lock.acquire()
        if provider_ip in self._peers_not_responded:
            self._peers_not_responded.remove(provider_ip)
        self._found_responses[(find_response.get_hash(), find_response.get_name())] = find_response
        self._finding_lock.release()
